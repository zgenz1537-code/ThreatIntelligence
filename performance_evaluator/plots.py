...
def set_common(ax, kwargs, CONFIG):
    ...
    if kwargs.get("xlabel", CONFIG["xlabel"]):
        ax.set_xlabel(
            kwargs.get("xlabel", CONFIG["xlabel"]),
    ...
    if kwargs.get("ylabel", CONFIG["ylabel"]):
        ax.set_ylabel(
            kwargs.get("ylabel", CONFIG["ylabel"]),
            labelpad=kwargs.get("xylabelpad", CONFIG["xylabelpad"]),
            fontdict=kwargs.get("xylabel_fontdict", CONFIG["xylabel_fontdict"]),
        )
    return ax
...
def confusion_matrix(actual, predicted, classes, ax=None, **kwargs):
    ...
    if kwargs.get("cbar", CONFIG["cbar"]):
        ...
        cbar.ax.set_yticklabels(
            ...
            fontdict=kwargs.get(
                "cbar_ticklabels_fontdict", CONFIG["cbar_ticklabels_fontdict"]
            ),
        )
    set_common(ax, kwargs, CONFIG)
    ax.set_xticklabels(
        classes,
        rotation=kwargs.get("xticklabels_rotation", CONFIG["xticklabels_rotation"]),
        fontdict=kwargs.get("ticklabels_fontdict", CONFIG["ticklabels_fontdict"]),
    )
    ax.set_yticklabels(
        classes,
        rotation=kwargs.get("yticklabels_rotation", CONFIG["yticklabels_rotation"]),
        fontdict=kwargs.get("ticklabels_fontdict", CONFIG["ticklabels_fontdict"]),
    )
    plt.tight_layout()
...
def precision_recall_curve(actual, probability, classes, ax=None, **kwargs):
    ...
    for i in range(probability.shape[1]):
        ...
        color = plt.get_cmap(kwargs.get("cmap", CURRENT_CMAP))(float(i) / len(classes))
        ax.plot(
            recall,
            precision,
            label="{0} (AP={1})".format(classes[i], round(ap_score_, 4)),
            color=color,
        )
    ...
    ax.legend(
        prop=kwargs.get("legend_fontdict", CONFIG["legend_fontdict"]),
...
def roc_curve(actual, probability, classes, ax=None, **kwargs):
    ...
    for i in range(probability.shape[1]):
        ...
        ax.plot(
            fpr,
            tpr,
            label="{0} (AUC={1})".format(classes[i], round(auc_score, 4)),
            color=color,
        )
    ax = set_common(ax, kwargs, CONFIG)
    ax.grid(which="major", alpha=0.7)
