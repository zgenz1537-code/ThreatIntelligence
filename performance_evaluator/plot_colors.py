...
palettes = [
    ...
    'icefire_r', 'inferno', 'inferno_r', 'magma', 'magma_r', 'mako', 'mako_r', 'nipy_spectral', 'nipy_spectral_r',
...
f, ax = plt.subplots(len(palettes) // 2, 2)
for i in range(0, len(palettes), 2):
    palette1 = sbn.color_palette(palettes[i])
    palette2 = sbn.color_palette(palettes[i + 1])
    try:
        ax[i][0].imshow(np.arange(len(palette1)).reshape(1, len(palette1)),
                        cmap=colors.ListedColormap(list(palette1)), interpolation="nearest", aspect="auto")
        ax[i][1].imshow(np.arange(len(palette2)).reshape(1, len(palette2)),
                        cmap=colors.ListedColormap(list(palette2)), interpolation="nearest", aspect="auto")
    except IndexError:
        pass
plt.show()
